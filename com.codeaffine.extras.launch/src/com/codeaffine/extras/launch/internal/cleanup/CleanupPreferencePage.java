...
public class CleanupPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
  ...
  private void createPageControls(Composite parent) {
    ...
    String text = "Note: Launch configurations are considered as on-the-fly generated if "
  ...
  private void layoutPageControls(Composite parent) {
    ...
    GridDataFactory.swtDefaults().align(SWT.FILL, SWT.TOP).grab(true, false).applyTo(parent);
    GridDataFactory.swtDefaults().align(SWT.FILL, SWT.TOP).indent(20, 0).span(2, 1).applyTo(cleanupTypesLabel);
    GridDataFactory.swtDefaults().align(SWT.FILL, SWT.TOP).indent(20, 0)
        .hint(SWT.DEFAULT, cleanupTypesViewer.getTable().getItemHeight() * 14).grab(true, false).span(1, 2)
        .applyTo(cleanupTypesViewer.getControl());
    GridDataFactory.swtDefaults().align(SWT.FILL, SWT.TOP)
        .hint(computePreferredButtonWidth(selectAllButton), SWT.DEFAULT).applyTo(selectAllButton);
    GridDataFactory.swtDefaults().align(SWT.FILL, SWT.TOP)
        .hint(computePreferredButtonWidth(deselectAllButton), SWT.DEFAULT).applyTo(deselectAllButton);
    GridDataFactory.swtDefaults().align(SWT.FILL, SWT.TOP).hint(getTextWidth(cleanupButton.getText()), SWT.DEFAULT)
        .span(2, 1).applyTo(notelabel);
  }
  ...
  private ILaunchConfigurationType[] getCheckedLaunchConfigTypes() {
    return Stream.of(cleanupTypesViewer.getCheckedElements()).map(element -> (ILaunchConfigurationType) element)
